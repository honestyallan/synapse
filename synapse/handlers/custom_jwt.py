...
class CustomJwtHandler:
    ...
    def verify_id_token(self, token) -> dict:
        ...
        jwk_set = self._jwks
        try:
            claims = jwt.decode(token, key=jwk_set)
        except ValueError:
            logger.info("Reloading JWKS after decode error, try reloading the jwks")
            # Ensure we have the actual JWKS set, not a retry object
            jwk_set = RetryOnExceptionCachedCall(self._load_jwks)()
            claims = jwt.decode(token, key=jwk_set)

        logger.debug("Decoded JWT %r; validating", claims)
        try:
            claims.validate(now=self._clock.time(), leeway=120)  # allows 2 min of clock skew
    ...
    def validate_login(self, login_submission: JsonDict) -> str:
        ...
        try:
            ...
            user_id = decoded_token['sub'].lower()
        except ValueError as e:
            # A specific JWT error occurred, provide detailed info.
            raise LoginError(
                403,
                "Custom JWT Token validation failed due to a value error: %s" % (str(e),),
                errcode=Codes.FORBIDDEN,
            )
        except Exception as e:
            # A generic error occurred during JWT validation.
            raise LoginError(
                403,
                "Custom JWT Token validation failed with an unexpected error: %s" % (str(e),),
                errcode=Codes.FORBIDDEN,
            )
